...
- New accounts never get a password typed by the admin. `UserForm`'s password field is `visible` only on `edit`; on `create`, `CreateUser::mutateFormDataBeforeCreate()` fills in a random unusable one (`Str::random(40)`). Instead, `CreateUser::afterCreate()` sends a `WelcomeNotification` (mail only, not queued — sent synchronously, no queue worker) containing a "set your password" link, reusing Laravel's own password-broker token system (the `password_reset_tokens` table Laravel already ships) rather than anything bespoke. That link is handled by the `SetPassword` Livewire page (`/set-password/{token}?email=`), which calls `Password::reset()` and logs the user in directly on success — no separate login step needed after setting a password. Token expiry is bumped to 3 days (`config/auth.php` → `passwords.users.expire`) since this is a welcome link a new hire might not open right away, not a security-sensitive forgot-password reset. If a link expires or gets lost, the admin has a "Resend welcome email" action on the Users table (`UsersTable.php`) rather than a dead end.
...
- The admin Files/Folders resources are flat, filterable/searchable tables (owner column, "Location" = direct parent name, `TrashedFilter` + restore/force-delete), not a click-through tree browser — that richer navigation experience is deliberately reserved for the regular-user "My Files" page. Creating a file from the admin side uses a `FileUpload` form field that is NOT dehydrated into the `files` table directly: `CreateFile::mutateFormDataBeforeCreate()` strips it into `$this->uploadedPath`, then `afterCreate()` builds the first `file_versions` row from it — same pattern as `MyFiles::uploadFile()`, just admin-side.
